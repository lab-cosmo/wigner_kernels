...
from torch_nice.clebsch_gordan import get_real_clebsch_gordan
...
class ClebschCombiningSingleUnrolled(torch.nn.Module):
    def __init__(self, clebsch, lambd):
        ...
        self.register_buffer(
            'clebsch',
            torch.from_numpy(clebsch).type(torch.get_default_dtype()))
        self.lambd = lambd
        self.l1 = (self.clebsch.shape[0] - 1) // 2
        self.l2 = (self.clebsch.shape[1] - 1) // 2
        transformation = get_real_clebsch_gordan(clebsch, self.l1, self.l2,
                                                 lambd)
        ...
        self.multipliers = torch.tensor(self.multipliers).type(
    ...
    def forward(self, X1, X2):
        ...
        contributions = X1[self.m1_aligned, :, :] * X2[
            self.m2_aligned, :, :] * multipliers[:, None, None]

        result = torch.zeros([2 * self.lambd + 1, X2.shape[1], X1.shape[2]],
                             device=device)
    ...
    def forward(self, X1, X2):
        ...
        if not self.has_task:
            if self.lambd == 0:
                ...
                result = torch.bmm(first, second) * self.multiplier
                #print(result.shape)
    ...
    def __init__(self, clebsch, lambd_max, task=None):
        ...
        for l1 in range(self.clebsch.shape[0]):
            for l2 in range(self.clebsch.shape[1]):
                for lambd in range(abs(l1 - l2),
                                   min(l1 + l2, self.lambd_max) + 1):
                    key = '{}_{}_{}'.format(l1, l2, lambd)

                    if lambd >= clebsch.shape[2]:
                        raise ValueError(
                            "insufficient lambda max in precomputed Clebsch Gordan coefficients"
                        )
                    if task is None:
                        task_now = None
                    else:
                    ...
                    if (task_now is None) or (len(task_now[0]) > 0):
                        self.single_combiners[key] = ClebschCombiningSingle(
                            clebsch[l1, l2, lambd, :2 * l1 + 1, :2 * l2 + 1],
                            lambd, task_now)

    def forward(self, X1_initial: Dict[str, torch.Tensor],
                X2_initial: Dict[str, torch.Tensor]):
        X1 = {}
